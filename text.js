...
export default async function getText(parashat, order = 'pasuk', showRashi = false) {
  let { chumash, start: [perekStart, pasukStart], end: [perekEnd, pasukEnd] } = parshiyot[parashat];
  ...
  while (perekEnd > perekStart || (pasukEnd >= pasukStart && perekEnd === perekStart)) {
    ...
    if (order === 'pasuk') {
      text += `<div>${perek ? `<span class="perek">${perek}</span>` : ''} <span class="pasuk">${pasuk}</span> <span class="torah">${torah}
      ${torah}</span>
      <span class="targum">${targum}</span>${rashi && showRashi ? `
      <span class="rashi">${rashi}</span>` : ''}</div>`;
    } else {
      parasha += `${perek ? `<span class="perek">${perek}</span>` : ''} <span class="pasuk">${pasuk}</span> ${torah} `;
      ...
      if (torah.endsWith('(ס)') || torah.endsWith('(פ)') || (pasukStart === pasukEnd && perekStart === perekEnd)) {
        text += `<div><span class="torah">${parasha}</span>  <span class="torah">${parasha}</span> <span class="targum">${parashaTrgum}</span>${parashaRashi && showRashi ? `<span class="rashi">${parashaRashi}</span>` : ''}</div>`;
        parasha = '';
        parashaTrgum = '';
        parashaRashi = '';
      }
